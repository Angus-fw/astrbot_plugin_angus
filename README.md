...
### 🤖 主动对话系统
- 定时触发：每分钟检查一次，根据设定的概率随机触发对话
- 触发语句管理：支持查看、添加和删除触发语句
- 动态配置：可通过命令动态调整触发概率
- 目标用户指定：可指定目标用户发送消息
...
| 功能 | 命令格式 | 示例 |
|---|---|---|
| 添加提醒 | `添加提醒 <内容> <时间> [开始星期] [重复类型] [--holiday_type=...]` | `添加提醒 写周报 8:05 mon weekly` |
| 添加任务 | `添加任务 <内容> <时间> [开始星期] [重复类型] [--holiday_type=...]` | `添加任务 发送天气预报 8:00 daily` |
| 查看全部 | `列表全部` |  |
| 删除指定 | `删除全部 <序号>` | `删除全部 1` |
| 帮助 | `帮助` |  |
